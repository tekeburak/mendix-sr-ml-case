...
def input_fn(request_body, request_content_type):
    if request_content_type == 'application/json':
        ...
        with open(os.path.join('processed_data', 'test_dataset.pkl'), 'wb') as file:
...
with open(os.path.join('processed_data', 'test_dataset.pkl'), 'rb') as file:
    dataset = pickle.load(file)
    
model = model_fn(model_dir='.')
pred = predict_fn(input_data=dataset, model=model)

# Initialize an empty DataFrame
columns = ['graphId', 'nodeId', 'prediction']  # Define column names
df = pd.DataFrame(columns=columns)

# Add rows iteratively to the DataFrame
for row_value in pred:  # Change the range to the number of rows you want to add
    df.loc[len(df)] = row_value  # Append the row to the DataFrame
    
    
# Write the DataFrame to a Parquet file
df.to_parquet('submission_test.parquet', index=False)